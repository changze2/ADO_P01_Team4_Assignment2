...
def fetch_api_data(**kwargs):
    ...
    cursor = conn.cursor()
    query = "SELECT SESSION_KEY FROM SESSIONS_API"

    
    # Execute the query
    cursor = conn.cursor()
    cursor.execute(query)
    session_keys = [row[0] for row in cursor.fetchall()]  # Fetch session keys as a list
    ...
    try:
        ...
        for key in meeting_keys:
            for k in session_keys:
                api_url = f"https://api.openf1.org/v1/position?session_key={k}&meeting_key={key}"
...
def load_to_snowflake(**kwargs):
    ...
    # Example usage
    insert_query = generate_insert_query('CAR_DATA_API', schema_dict)
    print(insert_query)
